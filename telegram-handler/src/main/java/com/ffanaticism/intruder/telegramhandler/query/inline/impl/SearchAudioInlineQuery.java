...
@Component
public class SearchAudioInlineQuery implements BaseInlineQuery {
    ...
    @Autowired
    public SearchAudioInlineQuery(VideoService videoService, BotService botService) {
    ...
    public void execute(User user, InlineQuery inlineQuery) {
        ...
        videoService.search(query, offset)
                .thenAccept(results -> botService.sendSearchAudioAnswerInlineQuery(
                        inlineQuery.getId(),
                        results,
                        offset,
                        RESULT_SIZE,
    ...
    public InlineQueryType getInlineQueryType() {
        return InlineQueryType.SEARCH_AUDIO;
    }
}
